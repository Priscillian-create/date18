  ...
  const dataManager = {
    ...
    serializeForSupabase: (table, data) => {
      ...
      if (table === 'transactions') {
        const safeUserId = (typeof d.user_id === 'string' && (d.user_id.startsWith('offline_') || d.user_id === 'offline_user')) ? null : d.user_id;
        return { user_id: safeUserId, user_email: d.user_email, section: d.section, type: d.type, amount: d.amount, charge: d.charge, pos_charge: d.pos_charge, reference: d.reference, customer_phone: d.customer_phone, timestamp: d.timestamp, notes: d.notes };
      }
    ...
    btn.addEventListener('click', async () => {
      if (!deferredPrompt) {
        ...
        if (!isStandalone && isIOS) {
          ...
          utils.showNotification('Install not available yet. Use HTTPS or localhost. Icons are optional.', 'info');
        }
        return;
      }
